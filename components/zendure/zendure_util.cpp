...
void publish_state_if_changed(esphome::sensor::Sensor *sensor, float value) {
  ...
  // close enough
  if (sensor->has_state() && fabs(sensor->state - value) < 0.0001)
    return;
...
void handle_sensor_object(JsonObject &obj, const sensor_handler_map_t &handlers,
  ...
  for (JsonObject::iterator it = obj.begin(); it != obj.end(); ++it) {
    std::string_view key(it->key().c_str());
    int value = it->value().as<int>();

    auto handler = handlers.find(key);
    if (handler != handlers.end()) {
      // 99% of values in zendures JSON are integers, so far only serial_numbers
